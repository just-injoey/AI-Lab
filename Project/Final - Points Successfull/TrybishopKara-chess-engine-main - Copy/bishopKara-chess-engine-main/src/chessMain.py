...
def main():
    ...
    loadImages()
    userPoints = 1000 # Initially the users will have 1000 points
    ...
    while running:
                    ...
                    if  len(playerClicks) == 2:# user wants to move a piece or 2nd click
                        ...
                        for i in range(len(validMoves)):
                            if move == validMoves[i]:
                                ...
                                ##############
                                print("User Moved: ",validMoves[i].pieceMoved)
                                if(validMoves[i].pieceMoved[1]=='K'):
                                    userPoints -= 10
                                elif(validMoves[i].pieceMoved[1]=='R'):
                                    userPoints -= 20
        ...
        if gs.checkmate:
            gameOver = True
            if gs.whiteToMove:
                if(not stopScoreFlag):
                    userPoints -= 100
                    print("User Points: ",userPoints)
                    stopScoreFlag = True
                drawText(screen, f"Black wins by checkmate, Score: {userPoints}")
            else:
                if(not stopScoreFlag):
                    userPoints += 100
                    print("User Points: ",userPoints)
                    stopScoreFlag = True
                drawText(screen, f"White wins by checkmate, Score: {userPoints}" )
        elif gs.stalemate:
            gameOver = True
